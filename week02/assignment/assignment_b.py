...
import threading
from cse251functions import * # says Import "cse251functions" could not be resolved
...
def main():
    ...
    assert x.product == 24, f'The product should equal 24 but instead was {
        x.product}'
    
    # Repeat, passing in 10 (delete this line).
    x.start(10)
    assert x.product == 362880, f'The product should equal 362880 but instead was {
        x.product}'

    # Repeat, passing in 15 (delete this line).
    x.start(15)     
    assert x.product == 87178291200, f'The product should equal 87178291200 but instead was {
        x.product}'
